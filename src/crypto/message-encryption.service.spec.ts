...
describe('MessageEncryptionService', () => {
  ...
  describe('encryptMessage', () => {
    it('should encrypt a message successfully', async () => {
      ...
      mockKeyExchangeService.getConversationKeys.mockResolvedValue(mockConversationKeys);
      ...
      expect(mockCryptoService.deriveMessageKey).toHaveBeenCalledWith('shared-secret', 6);
      expect(mockCryptoService.encryptMessage).toHaveBeenCalledWith(content, 'message-key');
      expect(mockKeyExchangeService.updateParticipantSequence).toHaveBeenCalledWith(
        conversationId,
        senderId,
        6,
      );
    ...
    it('should throw error if key exchange not completed', async () => {
      ...
      await expect(
        service.encryptMessage(conversationId, senderId, content, privateKey, privateSigningKey),
  ...
  describe('decryptMessage', () => {
    it('should decrypt a message successfully', async () => {
      ...
      const result = await service.decryptMessage(conversationId, encryptedMessage);
